...
class App
{
    ...
    private function __construct($app_language, $entityManager)
    {
        ...
        $this->_array_words = parse_ini_file($serverRoot.'/i18n/' . strtolower($app_language) . '.ini');

        /** Load ini file for each module in a global array */
        $words = array();
        $moduleRepository = $this->_entityManager->getRepository('Module');
        $modules = $moduleRepository->findAll();
        foreach ($modules as $module) {
            $words[$module->getModName()] = parse_ini_file($serverRoot.'/modules/' . $module->getModName() . '/i18n/' . $app_language . '.ini');
        }
    ...
    public static function getInstance($app_language, $entityManager)
    {
        ...
        if (is_null(self::$_instance)) {
            self::$_instance = new App($app_language, $entityManager);
        }
